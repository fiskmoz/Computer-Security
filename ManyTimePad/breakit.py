# MADE BY: 
# Christoffer Sundqvist 
# FOR: 
# computer security II at Karlstad University

import string

def main():
    c1 = ""
    c2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
    c3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
    c4 = ""
    c5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
    c6 = ""
    c7 = ""
    c8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
    c9 = ""
    c10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
    ciphers = [c1, c2, c3, c4, c5, c6, c7, c8, c9, c10]
    
    targetCipher = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"
    
    keystream = [None]*200
    KnownPositions = [None]*200
    
    # Enumerate all ciphertexts
    for firstIndex, ciphertext in enumerate(ciphers): 
        PotentialSpaces = [0]*200
        KnownSpaceIndexes = []
        # Enumerate all ciphertexts for a single ciphertext.
        for secondIndex, ciphertext2 in enumerate(ciphers): 
            # Do not XOR a cipher with itself.
            if not firstIndex is secondIndex: 
                # Enumeraate all hex values for XOR
                for indexOfChar, char in enumerate(strXOR(ciphertext.decode('hex'), ciphertext2.decode('hex'))): 
                    # check if it is a space or has the potential to be.
                    if char in string.printable and char.isalpha(): 
                        PotentialSpaces[indexOfChar] = PotentialSpaces[indexOfChar] + 1         
        
        # Enumerate the potential spaces
        for index, value in enumerate(PotentialSpaces): 
        # If it has been a space in 7 or more of the ciphertexts we assume it is a space
            if value > 6: 
                KnownSpaceIndexes.append(index)
                KnownPositions[index] = 1
        # XOR the ciphertexts with only spaces to get incomplete key 
        SpaceXOR = strXOR(ciphertext.decode('hex'),' '*200 )
        # Enumerate the known spaces indexes and get the accual values from they incomplete key and store them
        for index in KnownSpaceIndexes:
            keystream[index] = SpaceXOR[index].encode('hex')

    # Use the semi complete key and XOR with desired cipher to get something resembling the plaintext.
    keystreamhex = ''.join([value if value is not None else '00' for value in keystream])
    ourtext = strXOR(targetCipher.decode('hex'), keystreamhex.decode('hex'))
    print ''.join([text if KnownPositions[index] is 1 else '*' for index, text in enumerate(ourtext)])
    
    # Using the broken plaintext we can manually guess the correct message
    guess = raw_input("Guess the message to derive key: ")
    print(guess)
    # Use the presumably correctly guessed message we derive the key to then derive the other texts.
    key = strXOR(targetCipher.decode('hex'), guess)
    for cipher in ciphers:
        print strXOR(cipher.decode('hex'), key)

def strXOR(a,b):
    if len(a) > len(b):
        return "".join([chr(ord(x)^ord(y)) for (x,y) in zip(a[:len(b)],b)])
    else:
        return "".join([chr(ord(x)^ord(y)) for (x,y) in zip(a,b[:len(a)])])
        
main()